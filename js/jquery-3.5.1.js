...
jQuery.extend ({
	...
	isPlainObject: function (obj) {
		...
		// Objetos sem protótipo (por exemplo, ʻObject.create (null) `) são simples
		if (! proto) {
			return true;
		}
...
function isArrayLike (obj) {
	...
	// verificação ʻin` usada para evitar erro JIT (gh-2145)
...
function createCache () {
	...
	cache de função (chave, valor) {
		...
		// Use (tecla + "") para evitar a colisão com propriedades de protótipo nativo (consulte a edição nº 157)
...
function createDisabledPseudo (disabled) {
	...
	// Conhecido: falsos positivos desativados: fieldset [desativado]> legenda: enésimo de tipo (n + 2): pode-desativar
	função de retorno (elem) {
...
setDocument = Sizzle.setDocument = function (node) {
	...
	// Então, permitimos: foco em passar pelo QSA o tempo todo para evitar o erro do IE
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (função (el) {
			...
			// FF 3.5 -: ativado /: desativado e elementos ocultos (elementos ocultos ainda estão ativados)
			// IE8 gera erro aqui e não verá testes posteriores
			...
			retornar primeiro === 1 && último === 0?
				...
				function (elem, _context, xml) {
					var cache, uniqueCache, outerCache, node, nodeIndex, start,
						dir = simples! == avançar? "nextSibling": "previousSibling",
					...
					if (pai) {
						...
						if (forward && useCache) {
							...
							// Buscar ʻelem` de um índice previamente armazenado em cache
							...
							while ((node ​​= ++ nodeIndex && node && node [dir] ||
								...
								// Retorne à busca de ʻelem` desde o início
							...
							if (diff === falso) {
								...
								// Use o mesmo loop acima para buscar ʻelem` desde o início
								while ((node ​​= ++ nodeIndex && node && node [dir] ||
									(diff = nodeIndex = 0) || start.pop ())) {
...
function matcherFromGroupMatchers (elementMatchers, setMatchers) {
	var bySet = setMatchers.length> 0,
		...
		superMatcher = function (seed, context, xml, results, outermost) {
			...
			// ʻi` agora é a contagem dos elementos visitados acima, e adicionando-o a `matchedCount`
			...
			// é igual a ʻi`), a menos que não tenhamos visitado _qualquer_ elementos no loop acima porque temos
			// sem correspondências de elemento e sem semente.
			// Incrementar uma string inicial "0" ʻi` permite que ʻi` permaneça uma string apenas naquele
			// caso, o que resultará em um "00" `matchedCount` que difere de ʻi`, mas também é
			// numericamente zero.
			if (bySet && i! == matchedCount) {
				...
				if (seed) {
					...
					// Descarte os valores de espaço reservado do índice para obter apenas correspondências
...
select = Sizzle.select = function (seletor, contexto, resultados, semente) {
	...
	// Fornece `match` para evitar retokenização se modificarmos o seletor acima
...
jQuery.Callbacks = função (opções) {
	...
	var // Sinalizar para saber se a lista está disparando no momento
		...
		fogo = função () {
			...
			// respeitando as substituições firingIndex e mudanças de tempo de execução
				...
				pipe: function (/ * fnDone, fnFail, fnProgress * /) {
					...
					return jQuery.Deferred (function (newDefer) {
						jQuery.each (tuplas, função (_i, tupla) {
							...
							// deferred.prog ress (function () {bind to newDefer ou newDefer.notify})
...
if (document.readyState === "completo" ||
	...
	// Trate isso de forma assíncrona para permitir que os scripts tenham a oportunidade de ficar prontos
...
Data.prototype = {
	...
	remove: function (owner, key) {
		...
		if (chave! == undefined) {
			...
			// Suporta cadeia de chaves separadas por espaço
...
jQuery.fn.extend ({
	dados: função (chave, valor) {
		...
		acesso de retorno (isto, função (valor) {
			...
			// resultará em ʻundefined` para elem = this [0] que irá
...
jQuery.fn.extend ({
	...
	// Obtenha uma promessa resolvida quando há filas de um certo tipo
...
var documentElement = document.documentElement;
	...
	// As primeiras versões do iOS 10 suportam ʻattachShadow`, mas não `getRootNode`,
...
function getAll (context, tag) {
	...
	// Use typeof para evitar a invocação de método de argumento zero em objetos host (# 15151)
	var ret;
...
jQuery.event = {
	...
	add: function (elem, types, handler, data, selector) {
		...
		// Inicia a estrutura do evento do elemento e manipulador principal, se este for o primeiro
...
// para a descrição do bug (também existia em versões mais antigas do Chrome).
jQuery.each ({
...
(função () {
	...
	jQuery.extend (suporte, {
		...
		// definido em CSS enquanto as propriedades ʻoffset * `relatam os valores corretos.
	...
	para (; i <4; i + = 2) {
		...
		if (! isBorderBox) {
			...
			// Mas ainda mantenha o controle de outra forma
				...
				extra + = jQuery.css (elem, "borda" + cssExpand [i] + "Largura", verdadeiro, estilos);
			}
...
jQuery.extend ({
	...
	css: função (elem, nome, extra, estilos) {
		...
		// Experimente o nome com prefixo seguido do nome sem prefixo
		hooks = jQuery.cssHooks [nome] || jQuery.cssHooks [origName];
...
jQuery.each (["altura", "largura"], função (_i, dimensão) {
	jQuery.cssHooks [dimensão] = {
		...
		conjunto: função (elem, valor, extra) {
			...
			// Considere as dimensões de caixa de borda não confiáveis ​​comparando o deslocamento * com o calculado e
...
função Tween (elem, opções, prop, final, easing) {
...
Tween.prototype = {
	...
	init: function (elem, options, prop, end, easing, unit) {
		this.elem = elem;
		this.prop = prop;
		this.easing = atenuação || jQuery.easing._default;
...
function genFx (type, includeWidth) {
	...
	// caso contrário, o valor do passo é 2 para pular para a esquerda e direita
...
function defaultPrefilter (elem, props, opts) {
	...
	// Animações que ignoram a fila sequestram os ganchos fx
	if (! opts.queue) {
	...
	para (prop in orig) {
		...
		if (! propTween) {
			...
			// Armazena oculto / visível para alternar para `.stop (). Toggle ()` "reverter"
			if (alternar) {
...
função Animação (elem, propriedades, opções) {
	resultado var,
		...
		tick = function () {
			...
			para (; índice <comprimento; índice ++) {
				animation.tweens [índice] .run (porcentagem);
			}
	...
	para (; índice <comprimento; índice ++) {
		resultado = Animation.prefilters [índice] .call (animação, elem, adereços, animação.opts);
		if (resultado) {
...
jQuery.Animation = jQuery.extend (Animação, {
	...
	tweeners: {
		"*": [função (prop, valor) {
			...
			ajustar CSS (tween.elem, prop, rcssNum.exec (valor), tween);
			return tween;
		}]
	},
	...
	tweener: function (props, callback) {
		...
		var prop,
			índice = 0,
			comprimento = comprimento props.longo;
...
jQuery.speed = function (speed, easing, fn) {
	...
	// Vá para o estado final se fx estiver desligado
	if (jQuery.fx.off) {
		opt.duração = 0;
...
jQuery.fn.extend ({
		...
		return this.each (function () {
			...
			for (index = timers.length; index--;) {
				if (timers [index] .elem === this &&
					...
					cronômetros [índice] .anim.stop (gotoEnd);
					dequeue = false;
			...
			// Timers atualmente chamarão seus retornos de chamada completos, que
			// será retirado da fila, mas apenas se forem gotoEnd.
	...
	acabamento: função (tipo) {
		...
		return this.each (function () {
			...
			// Habilitar sinalizador de acabamento em dados privados
			data.finish = true;
			...
			for (index = timers.length; index--;) {
				if (timers [index] .elem === this && timers [index] .queue === type) {
					cronômetros [índice] .anim.stop (verdadeiro);
					timers.splice (índice, 1);
				}
			}
...
jQuery.each (["toggle", "show", "hide"], function (_i, name) {
	...
	jQuery.fn [nome] = função (velocidade, atenuação, retorno de chamada) {
		velocidade de retorno == null || typeof speed === "boolean"?
			cssFn.apply (this, argumentos):
			this.animado (genFx (nome, verdadeiro), velocidade, atenuação, retorno de chamada);
	};
});
...
jQuery.each ({
	...
	jQuery.fn [nome] = função (velocidade, atenuação, retorno de chamada) {
		retornar this.animado (adereços, velocidade, atenuação, retorno de chamada);
	};
});
...
jQuery.fn.delay = função (tempo, tipo) {
	time = jQuery.fx? jQuery.fx.speeds [tempo] || hora: hora;
	tipo = tipo || "fx";
...
(função () {
	...
	// Suporte: IE <= 11 apenas
	// Deve acessar selectedIndex para fazer as opções padrão selecionar
...
jQuery.fn.extend ({
	attr: function (nome, valor) {
		acesso de retorno (this, jQuery.attr, nome, valor, argumentos.length> 1);
	},
...
jQuery.extend ({
	attr: function (elem, nome, valor) {
		...
		if (valor! == undefined) {
			...
			if (hooks && "set" in hooks &&
				(ret = hooks.set (elem, valor, nome))! == undefined) {
				return ret;
			}
...
jQuery.fn.extend ({
	prop: função (nome, valor) {
		acesso de retorno (this, jQuery.prop, nome, valor, argumentos.length> 1);
	},

	removeProp: function (name) {
		return this.each (function () {
			exclua este [jQuery.propFix [name] || nome];
		});
	}
});
...
if (! support.optSelected) {
	jQuery.propHooks.selected = {
		get: function (elem) {
			...
			/ * eslint no-unused-extensions: "off" * /
...
jQuery.each ([
	...
	jQuery.propFix [this.toLowerCase ()] = isso;
});
	...
	removeClass: function (value) {
		...
		if (classes.length) {
			while ((elem = this [i ++])) {
				...
				// Esta expressão está aqui para melhor compactação (consulte addClass)
				cur = elem.nodeType === 1 && ("" + stripAndCollapse (curValue) + "");
	...
	val: função (valor) {
		...
		if (! arguments.length) {
			if (elem) {
				...
				// Lidar com casos de string mais comuns
				if (typeof ret === "string") {
					return ret.replace (rreturn, "");
				}

				// Trate os casos em que o valor é nulo / undef ou número
				return ret == null? "": ret;
			}
		...
		return this.each (function (i) {
			...
			if (! hooks ||! ("set" em hooks) || hooks.set (this, val, "value") === undefined) {
				this.value = val;
			}
		});
	}
...
jQuery.extend (jQuery.event, {
	...
	trigger: function (event, data, elem, onlyHandlers) {
		...
		var i, cur, tmp, bubbleType, ontype, handle, special, lastElement,
			...
			type = hasOwn.call (evento, "tipo")? event.type: event,
		...
		evento = evento [jQuery.expando]?
			evento:
			novo jQuery.Event (type, typeof event === "object" && event);
		...
		// Bolha para o documento e depois para a janela; observe um proprietário global do documento var (# 9724)
		if (! onlyHandlers &&! special.noBubble &&! isWindow (elem)) {
			...
			if (tmp === (elem.ownerDocument || document)) {
				eventPath.push (janela tmp.defaultView || tmp.parentWindow ||);
			}
		}
	...
	// Pega carona em um evento de doador para simular um diferente
...
jQuery.fn.extend ({
	...
	gatilho: função (tipo, dados) {
		return this.each (function () {
			jQuery.event.trigger (tipo, dados, isso);
		});
	},
	triggerHandler: function (type, data) {
		var elem = este [0];
		if (elem) {
			return jQuery.event.trigger (tipo, dados, elem, verdadeiro);
		}
	}
});
	...
	jQuery.each ({focus: "focusin", blur: "focusout"}, function (orig, fix) {
		...
		jQuery.event.special [fix] = {
			configuração: função () {
				...
				// Manipular: nós regulares (via `this.ownerDocument`), janela
				// (via `this.document`) e document (via` this`).
				var doc = this.ownerDocument || this.document || esta,
...
function buildParams (prefix, obj, traditional, add) {
	...
	if (Array.isArray (obj)) {
		...
		jQuery.each (obj, function (i, v) {
			if (tradicional || rbracket.test (prefixo)) {
				...
				// O item é não escalar (matriz ou objeto), codifique seu índice numérico.
				buildParams (
	...
	} else if (! traditional && toType (obj) === "objeto") {
...
// Uma extensão especial para opções de ajax
// isso leva opções "planas" (não devem ser estendidas profundamente)
// Correções # 9887
 ...
 * - encontra o dataType correto (medeia entre o tipo de conteúdo e o dataType esperado)
...
function ajaxHandleResponses (s, jqXHR, responses) {
	...
	if (dataTypes [0] nas respostas) {
		...
		// Tente c onvertible dataTypes
		para (digite as respostas) {
...
/ * Conversões em cadeia dada a solicitação e a resposta original
...
jQuery.extend ({
	...
	ajax: função (url, opções) {
				...
				statusCode: function (map) {
					...
					if (mapa) {
						if (concluído) {
							...
							// Execute os callbacks apropriados
							jqXHR.always (map [jqXHR.status]);
						} outro {

							// Preguiçoso-adicione os novos callbacks de uma forma que preserve os antigos
							para (código no mapa) {
		...
		// Adicionar protocolo se não fornecido (pré-filtros podem esperar isso)
		...
		// Podemos disparar eventos globais a partir de agora se solicitados
		...
		if (! s.hasContent) {
			...
			sem cache = s.url.slice (cacheURL.length);

			// Se os dados estão disponíveis e devem ser processados, anexe os dados ao url
			if (s.data && (s.processData || typeof s.data === "string")) {
			...
			if (s.cache === false) {
				...
				sem cache = (rquery.test (cacheURL)? "&": "?") + "_ =" + (nonce.guid ++) +
					não separado;
			}
		...
		if (s.beforeSend &&
			(s.beforeSend.call (callbackContext, jqXHR, s) === falso || concluído)) {
		...
		function done (status, nativeStatusText, responses, headers) {
			...
			// Use um conversor noop para script ausente
			if (! isSuccess && jQuery.inArray ("script", s.dataTypes)> -1) {
				s.converters ["script de texto"] = função () {};
			}
			...
			// Se for bem sucedido, lidar com o encadeamento de tipo
			if (isSuccess) {
			...
			if (fireGlobals) {
				...
				// Manipular o contador AJAX global
				if (! (--jQuery.active)) {
	...
	getJSON: function (url, data, callback) {
		retornar jQuery.get (url, dados, retorno de chamada, "json");
	},
...
jQuery._evalUrl = function (url, opções, doc) {
	return jQuery.ajax ({
		...
		// Torne isso explícito, pois o usuário pode sobrescrever isso por meio de ajaxSetup (# 11264)
		digite: "GET",
		dataType: "script",
		cache: verdadeiro,
		async: false,
		...
		// Avalie a resposta apenas se for bem-sucedida (gh-4126)
		// dataFilter não é chamado para respostas de falha, portanto, use-o
		// do conversor padrão é kludgy, mas funciona.
		conversores: {
...
jQuery.fn.extend ({
	...
	desembrulhar: função (seletor) {
		this.parent (seletor) .not ("body") .each (function () {
...
var xhrSuccessStatus = {
		...
		// O protocolo de arquivo sempre produz código de status 0, suponha 200
		0: 200,
	...
	if (support.cors || xhrSupported &&! options.crossDomain) {
		Retorna {
			enviar: função (cabeçalhos, completo) {
				...
				// semelhante a um quebra-cabeça, simplesmente nunca o configuramos para ter certeza.
				// (sempre pode ser definido por solicitação ou até mesmo usando ajaxSetup)
				// Para solicitações do mesmo domínio, não mudará o cabeçalho se já fornecido.
				...
				tentar {
					...
					// # 14683: Apenas relançar se isso não tiver sido notificado como um erro ainda
					if (callback) {
						jogue e;
					}
				}
...
jQuery.ajaxTransport ("script", função (ões) {
	...
	// Este transporte lida apenas com solicitações entre domínios ou forçadas por atrs
		...
		Retorna {
			enviar: função (_, completo) {
				script = jQuery ("<script>")
					...
					.on ("erro de carregamento", retorno de chamada = função (evt) {
						script.remove ();
				...
				// Use a manipulação DOM nativa para evitar nosso truque domManip AJAX
...
jQuery.ajaxPrefilter ("json jsonp", function (s, originalSettings, jqXHR) {
	...
	if (jsonProp || s.dataTypes [0] === "jsonp") {
		...
		// Insira o retorno de chamada no url ou nos dados do formulário
		if (jsonProp) {
...
// Por isso, essa medida de segurança deve ser desativada no Safari 8.
...
jQuery.parseHTML = function (data, context, keepScripts) {
	...
	if (! contexto) {
		...
		// Impede que scripts ou manipuladores de eventos inline sejam executados imediatamente
		...
		if (support.createHTMLDocument) {
			...
			// baseiam-se no URL do documento (gh-2965)
...
jQuery.fn.load = function (url, params, callback) {
	...
	if (off> -1) {
		seletor = stripAndCollapse (url.slice (desligado));
		url = url.slice (0, desligado);
	}
	...
	if (self.length> 0) {
		jQuery.ajax ({
			...
			// Se a variável "tipo" for indefinida, então o método "GET" será usado.
...
jQuery.offset = {
	setOffset: function (elem, options, i) {
		...
		CalculePosição = (posição === "absoluta" || posição === "fixa") &&
		...
		if ("usando" nas opções) {
			options.using.call (elem, adereços);

		} outro {
			if (typeof props.top === "número") {
				props.top + = "px";
			}
			if (typeof props.left === "número") {
				props.left + = "px";
			}
...
jQuery.fn.extend ({
	...
	deslocamento: função (opções) {
		...
		// Preserva o encadeamento para setter
		if (arguments.length) {
		...
		if (! elem.getClientRects (). comprimento) {
			return {top: 0, left: 0};
		}

		// Obter posição relativa ao documento adicionando rolagem da janela de visualização ao gBCR relativo à janela de visualização
	...
	// position () relaciona a caixa de margem de um elemento com a caixa de preenchimento do pai de deslocamento
	...
	posição: função () {
		...
		// posição: os elementos fixos são deslocados da janela de visualização, que sempre tem deslocamento zero
		if (jQuery.css (elem, "posição") === "fixo") {
			...
			// Considere o pai de deslocamento * real *, que pode ser o documento ou seu elemento raiz
			...
			if (offsetParent && offsetParent! == elem && offsetParent.nodeType === 1) {
				...
				// Incorpora bordas em seu deslocamento, uma vez que estão fora de sua origem de conteúdo
...
jQuery.each ({Altura: "altura", Largura: "largura"}, função (nome, tipo) {
	jQuery.each ({preenchimento: "interno" + nome, conteúdo: tipo, "": "externo" + nome},
		...
		jQuery.fn [funcName] = função (margem, valor) {
			...
			}, tipo, acorrentável? margem: indefinida, encadeada);
		};
	});
});
...
jQuery.fn.extend ({
	...
	vincular: função (tipos, dados, fn) {
		retornar this.on (tipos, nulos, dados, fn);
	},
	...
	pairar: function (fnOver, fnOut) {
		retornar this.mouseenter (fnOver) .mouseleave (fnOut || fnOver);
	}
});

jQuery.each (("desfocar foco foco em foco fora redimensionar scroll click dblclick" +
...
// No entanto, não está previsto para remoção em breve
jQuery.proxy = function (fn, contexto) {
	var tmp, args, proxy;
...
jQuery.isNumeric = function (obj) {
	...
	return (type === "número" || type === "string") &&
		...
		// ... mas interpreta erroneamente strings de números iniciais, especialmente literais hexadecimais ("0x ...")
...
// derivado de nomes de arquivo, e jQuery é normalmente entregue em minúsculas
...
// AMD loader está presente. jQuery é um caso especial. Para mais informações, veja
...
var
	...
	// Mapear sobre $ em caso de sobrescrever
	_ $ = janela. $;
